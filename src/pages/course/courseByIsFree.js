...
import { getCourseByIsFree } from "../../services/course/course.service.js"

const CourseByIsFree = () => {
    const [courses, setCourses] = React.useState([]);
    const { isFree } = useParams();
    React.useEffect(() => {
        const fetchData = async () => {
            try
            {
                const result = await getCourseByIsFree(isFree);
                ...
                <section className="blog-wrap-layout9">
                    <div className="container">
                        <div className="section-heading-3">
                            <h2>Danh sách khoá học {isFree === 1 ? 'Miễn phí' : 'Có phí'}</h2>
                        </div>
